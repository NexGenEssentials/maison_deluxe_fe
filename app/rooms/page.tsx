"use client";
import React, { useEffect, useRef, useState } from "react";
import Tamplate from "../components/common/template";
import Image from "next/image";
import { MdFilterListAlt } from "react-icons/md";
import BookingForm from "../components/model/paymentModel";
import { IoClose } from "react-icons/io5";
import { getRoomByTitle } from "../utils/filters";
import { roomType } from "../costants";

const RoomsPage = ({ params }: { params: string }) => {
  const room = getRoomByTitle("Penthouse Suite");
  const [selectedImage, setSelectedImage] = useState(room.image);
  const [activeTab, setActiveTab] = useState("Room Description");
  const [openModel, setOpenModel] = useState(false);
  const modalRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        modalRef.current &&
        !modalRef.current.contains(event.target as Node)
      ) {
        setOpenModel(false);
      }
    };

    if (openModel) {
      document.addEventListener("mousedown", handleClickOutside);
    } else {
      document.removeEventListener("mousedown", handleClickOutside);
    }

    return () => {
      document.removeEventListener("mousedown", handleClickOutside);
    };
  }, [openModel]);

  return (
    <Tamplate>
      <>
        <div className="w-full h-screen bg-[#F5F5F5] text-black py-10 px-4 md:px-12 lg:px-24">
          {/* Title */}

          <div className="font-[Playfair_Display] bg-gradient-to-r from-secondaryBlue  to-primaryGreen  text-center  mb-10 bg-clip-text text-transparent font-semibold text-[min(10vw,48px)]">
            {room.title}
          </div>

          <div className="flex flex-col lg:flex-row gap-8">
            {/* Left: Image Slider */}
            <div className="relative flex-1 lg:flex-3/5 overflow-hidden">
              <div className="relative h-[550px] w-full rounded-xl overflow-hidden">
                <Image
                  src={selectedImage}
                  alt="Room Image"
                  fill
                  className="w-full object-cover rounded-xl"
                />
              </div>

              <div className="absolute bottom-0 backdrop-blur-[2px] w-fit p-4 flex gap-4 mt-4 overflow-hidden justify-center lg:justify-start">
                {roomType.slice(0, 3).map((img, idx) => (
                  <div
                    key={idx}
                    onClick={() => setSelectedImage(img.image)}
                    className={`border-1 ${
                      selectedImage === img.image
                        ? "border-primaryGreen/44 shadow-xs shadow-lime-600"
                        : "border-transparent"
                    } relative w-34 h-24 rounded-md overflow-hidden cursor-pointer transition`}
                  >
                    <Image src={img.image} alt={`Thumbnail ${idx}`} fill />
                  </div>
                ))}
              </div>
            </div>

            {/* Right: Description & Amenities */}
            <div className="flex-1 lg:flex-2/5 bg-gray-50 p-6 rounded-xl shadow">
              <div className="flex items-center justify-center space-x-4 mb-4 w-full bg-[#F5F5F5] p-3 rounded-md">
                {["Room Description", "Room Policies"].map((item, index) => (
                  <button
                    key={index}
                    onClick={() => setActiveTab(item)}
                    className={`text-sm font-medium ${
                      activeTab === item ? "bg-white rounded-md" : ""
                    } hover:text-gray-800 cursor-pointer flex-1 text-center p-3 transition duration-300`}
                  >
                    {item}
                  </button>
                ))}
              </div>
              {activeTab === "Room Description" && (
                <>
                  <p className="text-gray-700 mb-4">
                    {roomType[0].description}
                  </p>

                  <h3 className="font-bold text-[#353535] text-lg mb-2">
                    Room Amenities
                  </h3>
                  <ul className="text-gray-700 space-y-2  list-inside">
                    {roomType[0].amenities.map((amenity, index) => (
                      <li
                        key={index}
                        className="flex items-center gap-2 text-[#353535] font-normal"
                      >
                        <MdFilterListAlt />
                        {amenity}
                      </li>
                    ))}
                  </ul>

                  <button
                    onClick={() => setOpenModel(true)}
                    className=" text-primaryGreen mt-8 bg-primaryBlue/80   font-semibold px-6 py-3 rounded-xl cursor-pointer transition"
                  >
                    Book Now
                  </button>
                </>
              )}
              {activeTab === "Room Policies" && (
                <div className="text-gray-700 space-y-4 text-sm">
                  <h3 className="font-bold text-[#353535] text-lg mb-2">
                    Room Policies
                  </h3>
                  <ul className="text-gray-700 space-y-4 list-disc list-inside">
                    <li>
                      <span className="font-semibold">Check-in:</span> From 2:00
                      PM <span className="font-semibold">Check-out:</span> Until
                      11:00 AM
                    </li>
                    <li>
                      <span className="font-semibold">
                        Cancellation/Prepayment:
                      </span>{" "}
                      Policies vary by room type and booking conditions. Check
                      your booking confirmation for more information.
                    </li>
                    <li>
                      <span className="font-semibold">
                        Children & Extra Beds:
                      </span>{" "}
                      All children are welcome. Extra beds are available on
                      request (additional charges may apply).
                    </li>
                    <li>
                      <span className="font-semibold">Pets:</span> Pets are not
                      allowed.
                    </li>
                    <li>
                      <span className="font-semibold">Smoking:</span> Smoking is
                      prohibited inside the rooms. Designated smoking areas are
                      available.
                    </li>
                    <li>
                      <span className="font-semibold">
                        ID & Age Requirements:
                      </span>{" "}
                      Valid government-issued ID is required at check-in.
                      Minimum age to book a room is 18.
                    </li>
                    <li>
                      <span className="font-semibold">Quiet Hours:</span> 10:00
                      PM – 7:00 AM
                    </li>
                    <li>
                      <span className="font-semibold">Damage Deposit:</span> A
                      refundable security deposit may be required upon arrival.
                    </li>
                  </ul>
                </div>
              )}
            </div>
          </div>
          {/* Modal for Booking */}
        </div>

        {openModel && (
          <div className="fixed inset-0 bg-primaryBlue/50 flex justify-end z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg w-full md:w-3/4 lg:w-1/2  2xl:w-1/3 h-full overflow-y-auto">
              <div
                ref={modalRef}
                className="flex items-center justify-between mb-4"
              >
                <h2 className="text-xl font-semibold">Booking Form</h2>
                <button
                  onClick={() => setOpenModel(false)}
                  className=" text-red-500 font-bold  px-2 py-2 rounded"
                >
                  <IoClose size={20} />
                </button>
              </div>
              <BookingForm />
            </div>
          </div>
        )}
      </>
    </Tamplate>
  );
};

export default RoomsPage;
